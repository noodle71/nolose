...
const {format} = require('./syslogUtils.js');
...
class SysLogger extends EventEmitter {
   ...
   */
  send(message, severity, tag) {
    severity = severity || this.severity;
    tag = tag || this.tag;
    return this._send(message, severity, tag);
  }
   ...
   */
  warn() {
    return this._send(format.apply(this, arguments), SysLogger.Severity.warn);
  }
  ...
  time(label) {
    return this._times[label] = Date.now();
  }

  timeEnd(label) {
    const duration = Date.now() - this._times[label];
    return this.log('%s: %dms', label, duration);
  }
